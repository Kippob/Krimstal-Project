...
---@overload fun(...): VaporBusterSpell
local VaporBusterSpell, super = Class(Object)

function VaporBusterSpell:init(user,target)
    super.init(self)
    self.user = user
    self.target = target
    ---@type fun(self: VaporBusterSpell, hit_action_command: boolean)
    self.damage_callback = function () end
    self.clock = -0.05
    self.antispam = 0
    self.action_command_timer = math.huge
    self.action_command_threshold = 0.2
    self.slashes_count = 3
    self.slash_flipped = false
    self.damage_delay = 0
end

function VaporBusterSpell:update()
    super.update(self)
    ...
    if self.clock > 0 and self.slashes_count > 0 then
        self:generateSplash((self.slash_flipped) and -1 or 1)
...
function VaporBusterSpell:generateSplash(scale_x)
    local cutAnim = Sprite("effects/attack/splash")
