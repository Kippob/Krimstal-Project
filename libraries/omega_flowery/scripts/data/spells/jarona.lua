...
function spell:onCast(user, target)
    ...
    local omega = Game:getPartyMember("flowery"):getFlag("omega_color", false)

    Game:getPartyMember("flowery"):setFlag("hover", false)
    ...
    Game.battle.timer:after(1.5, function()
        ...
        if omega == false then
            ...
            user:setAnimation("battle/jarona")
        end
        user:slideTo(tx, ty + 80, 1, "in-quad")

        Game.battle.timer:everyInstant(0.125, function ()
            local afterimage = AfterImage(user.sprite, 0.25, 0.05)
            afterimage.debug_select = false
            afterimage.physics.speed_x = -1
            afterimage:setColor(user.sprite:getColor())
            if omega == true then
                afterimage:addFX(ColorMaskFX({user.sprite:getColor()}, 1))
            else
        ...
        Game.battle.timer:after(1, function()
            ...
            local function jarona()
                ...
                Game.battle.timer:tween(timer, user, {x = next_x, y = next_y})
            ...
            Game.battle.timer:after(4, function ()
                ...
                Game.battle.timer:after(0.75, function()
                    ...
                    if omega == true then
                        Game:getPartyMember("flowery"):setFlag("hover", true)
                    end
                end)
            end)
        end)
